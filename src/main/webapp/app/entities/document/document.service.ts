...
export class DocumentService {
  ...
  protected convertDateFromClient(document: IDocument): IDocument {
    const copy: IDocument = Object.assign({}, document, {
      issuedAt: document.issuedAt && document.issuedAt.isValid() ? document.issuedAt.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.issuedAt = res.body.issuedAt ? moment(res.body.issuedAt) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((document: IDocument) => {
        document.issuedAt = document.issuedAt ? moment(document.issuedAt) : undefined;
      });
    }
    return res;
  }
